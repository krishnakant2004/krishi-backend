...
router.post('/filter', asyncHandler(async (req, res) => {
    try {
        ...
        console.log("search field :", name);
        console.log("bodydata for:", bodyData);
        ...
        const [matchingCategories, matchingSubCategories] = await Promise.all([
            Category.find({ name: { $regex: regex } }).select('_id'),
            subCategory.find({ name: { $regex: regex } }).select('_id')
        ]);
        ...
        const products = await Product.find({
            $or: [
                ...
                { proSubCategoryId: { $in: subCategoryIds } }
            ]
        })
        .populate('proCategoryId', 'id name')
        .populate('proSubCategoryId', 'id name')
        ...
        console.error("Error in filterProduct API:", error);
        res.status(500).json({ success: false, message: error.message });
    }
}));
        ...
        uploadProduct.fields([
            ...
            fields.forEach((field, index) => {
                if (req.files[field] && req.files[field].length > 0) {
                    ...
                    const imageUrl = `https://krishi-backend-4iyt.onrender.com/image/products/${file.filename}`;
